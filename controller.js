...
document.getElementById("reviewWord").addEventListener("click", function() {
    reviewCurrentWord(); // 调用显示当前单词背诵时间的函数
});

function reviewCurrentWord() {
    let log = document.getElementById('log');
    log.innerHTML = "";  // 清空显示区域

    // 假设 currentWord 是你当前正在查看的单词
    let currentWord = Model.CET6[0]; // 示例，获取第一个单词，实际可以根据实际需要替换

    let wordDetails = `${currentWord.en} (${currentWord.pn}) - `;

    if (currentWord.timer) {
        let d = currentWord.timer;
